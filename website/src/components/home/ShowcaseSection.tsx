...
const showcaseItems = [
  { src: '/examples/1.png', alt: 'KLine Chart Screenshot 1', type: 'image' as const },
  { src: '/examples/2.png', alt: 'KLine Chart Screenshot 2', type: 'image' as const },
  { src: '/examples/demo.mp4', alt: 'KLine Chart Demo', type: 'video' as const },
];
          ...
          {showcaseItems.map((item, i) => (
            <StaggerItem key={i}>
              ...
              >
                ...
                <div className="relative aspect-[1179/2556] bg-bg-secondary flex items-center justify-center overflow-hidden">
                  {item.type === 'video' ? (
                    <video
                      src={item.src}
                      autoPlay
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <span className="inline-flex items-center px-3 py-1 rounded-full bg-accent/10 border border-accent/20 text-accent text-xs font-medium">
                    {item.type === 'video' ? 'Live Demo' : 'Screenshot'}
                  </span>
                </div>
              </motion.div>
            </StaggerItem>
